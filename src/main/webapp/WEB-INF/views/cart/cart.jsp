				...
				<c:when test="${fn:length(list) > 0}">
					<c:forEach items="${list }" var="list" varStatus="status">
						...
						<tr>
							...
							<td>
				  				<a href="/stu/shop/goodsDetail.do?IDX=${row.GOODS_NO }">${list.itemname}</a> <br>
				  				색상: ${row.GOODS_ATT_COLOR } <br> 
				  				사이즈:${row.GOODS_ATT_SIZE } <br>
				  			</td>
				  			<td style="text-align:center">
                  				<input type="number" name="basket_goods_amount" value="1" min="1" max="999" style="width:50px; text-align:right" >
                  			</td>
							<td style="text-align:center">
